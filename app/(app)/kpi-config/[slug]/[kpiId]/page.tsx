/**
 * Per-tile KPI configuration. Renders the existing ConfigureClient editor
 * filtered to a single KPI. Linked from each tile's gear icon on /clients,
 * and reachable via the picker on /kpi-config.
 */
...
import { ConfigureClient } from '@/app/(app)/reporting/[slug]/configure/configure-client'
import {
...
export default async function KPIConfigTilePage({
  params,
}: {
  params: Promise<{ slug: string; kpiId: string }>
}) {
  ...
  if (error || !client) {
    return (
      <div className="px-4 sm:px-6 lg:px-8 py-6 max-w-7xl mx-auto">
        <Link href="/kpi-config" className="inline-flex items-center text-sm text-slate-400 hover:text-slate-200 mb-4">
          <ChevronLeft className="h-4 w-4 mr-1" /> Back to KPI Config
        </Link>
